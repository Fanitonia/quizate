...
1. **Çıkış Yapma**
   ...
   - **Açıklama:** Giriş yapan kullanıcı hesabından çıkış yapar.
...
1. **Bir Quiz'e Erişme**
   ...
   - **Açıklama:** Seçilen quizin detayları alınır.

1. **Bir Quiz'i Silme**
   - **API Metodu:** `DELETE /quizzes/{quizId}`
   - **Açıklama:** Quiz sahibi veya admin tarafından quiz silinir.

1. **Bir Quiz'in Sorularını Alma**
   - **API Metodu:** `GET /quizzes/{quizId}/questions`
   - **Açıklama:** Seçilen quizin soruları alınır.
